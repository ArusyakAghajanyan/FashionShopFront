import { getOrders, getOrderByStatus, authoriseUser,get } from "../../services/api";
  ...
  async function orderShow() {
    try {
      ...
      if (user && user[`${domainName}roles`] === ADMIN) {
       ...
       const dataResult = await Promise.all([await getProducts(), await getOrders(user.sub, token, UNPAID)])
